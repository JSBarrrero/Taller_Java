    ...
    public static void consultarResultados() {
        ...
        if (RegistrarEstudiante.cont > 0) {
            for (Integer i=0;i<8;i++) {
                if (!ConsultarInscritos.incritosRealizado[i][1]) {
                    cont++;
                }
            }
            if (cont == 8) {
                System.out.println("Ningun estudiante aprobo el curso");
            }else{
                System.out.println("Los estudiantes que aprobaron el curso son:");
                for (Integer j=0;j<8;j++) {
                    if (ConsultarInscritos.incritosRealizado[j][1] == true) {
